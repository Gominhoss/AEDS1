...
TCelula *PesquisarPorNome (TLista Lista, TProduto Item)
{
    ...
    while (Aux->prox != NULL)
    {
        if(strcmp(Item.nome, Aux->item.nome) == 0)
            return Aux;
        Aux = Aux->prox;
    }
    return NULL;
...
int main()
{
    int opcao, QuantosProdutosQuerInserir, OpcaoDePesquisa;
    ...
    do
    {
        ...
        case 3:
            printf("\nQual metodo de pesquisa?");
            printf("\n1 - Codigo");
            printf("\n2 - Nome\n");

            fflush(stdin);
            scanf("%d", &OpcaoDePesquisa);
            ...
            case 2:
                ...
                AuxiliarDePesquisa = PesquisarPorNome(L1, x);
                if(AuxiliarDePesquisa==NULL)
                {
                    printf("\nNao foi possivel encontrar o produto");
                }
